...
#[derive(Serialize, Deserialize, Clone)]
struct DateInfo {
    solar_date: String,      // 公历日期，格式：2024年8月28日
    lunar_date: String,      // 农历日期，格式：甲辰年七月廿五
    weekday: String,         // 星期，格式：星期三
    lunar_year: String,      // 农历年份，格式：甲辰年（龙年）
    lunar_month: String,     // 农历月份，格式：七月
    lunar_day: String,       // 农历日期，格式：廿五
}
...
// Tauri 命令：获取当前日期信息
#[tauri::command]
async fn get_current_date() -> Result<DateInfo, String> {
